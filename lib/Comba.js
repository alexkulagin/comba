...
//┘
	...
	function CombaList (options)
	{
		...
		const
			_self = this,
			_instance = Object.setPrototypeOf((callback) => _instance.run(callback), _self);
			_instance.constructor = CombaList;


		let _tasks = (options.list.length) ? __prepareTasks(options.list) : options.list,
				...
				// ─────────────────────────────────────────────────
					...
					run:
					{
						value: (callback) =>
						{
							...
							runner.complete = (toString.call(callback) === '[object Function]') ? callback : null;

							const onRun = _listeners.run;
